  ...
  'group inline-flex h-9 w-max items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors focus:outline-none disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      active: {
        ...
        false: 'bg-background hover:bg-primary hover:text-primary-foreground',
      },
    },
    defaultVariants: {
      active: false,
...
>(({ className, children, active = false, ...props }, ref) => (
  <NavigationMenuPrimitive.Trigger
    ref={ref}
    className={cn(navigationMenuTriggerStyle({ active }), 'group', className)}
    {...props}
  >
    {children}{' '}
    <ChevronDownIcon
...
const NavigationMenuNavLink = React.forwardRef<
...
>(({ className, active = false, ...props }, ref) => (
  <NavigationMenuPrimitive.Link
    ref={ref}
    className={cn(navigationMenuTriggerStyle({ active }), className)}
    {...props}
  />
))
